	...
	<div class="clone-wrapper wrapper-<?= $field['complete_name'] ?>" data-field="<?= $field['complete_name'] ?>">
	    <?php if (isset($stored_data) && !empty($stored_data->{$field['complete_name']})){ ?>
	    	...
	    	<?php for($i=0; $i<count($steps);$i++){ ?>
				<div class="toclone"> 	
					...
					<a href="" class="delete btn btn-default pull-right">Remove</a>
					<a href="" class="clone btn btn-default btn-fill pull-right">Add</a>
	    ...
	    <?php }else{ ?>
		    <div class="toclone"> 
			    <div class="row">
				    <div class="col-md-6">
				    	<input type="text" class="info-step form-control" name="title" placeholder="Title" value="">
					</div>
					<div class="col-md-6">
